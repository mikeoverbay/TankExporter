...
void main(void) {
    // NEW: Make += deterministic across drivers
    gColor = vec4(0.0);
    blmColor = vec4(0.0);
    ...
    vec4 AO = vec4(1.0, 1.0, 1.0, 1.0); // default value if ao map missing
    if (ao_exist == 1) {
        AO = texture2D(aoMap, TC1.st);
    }
    ...
    // RESTORED: alpha test uses normal-map R channel (your pipeline)
    if (aRef > bumpMap.r) {
        discard;
    }
   ...
   // 3rd try at PBR lighting
    ...
    // GMM - Metallic and Roughness material properties are packed together
